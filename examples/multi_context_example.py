...
# Configurar logging más detallado para depuración
...
class SmartAgent(Agent):
    """Agente LLM capaz de mantener múltiples contextos de conversación."""
    # El comportamiento se inyectará desde main()
...
async def main():
    ...
    logger.info("Iniciando ejemplo de múltiples contextos - VERSIÓN CORREGIDA")
    ...
    llm_agent = SmartAgent(llm_jid, llm_password)
    ...
    # Crear el gestor de contexto con un prompt explícito sobre mantener contextos separados
    ...
    # Debug del sistema
    logger.debug("ContextManager creado con sistema de gestión de contextos múltiples")

    # Crear el comportamiento LLM
    llm_behaviour = LLMBehaviour(
        llm_provider=openai_provider,
        context_manager=context_manager,
        # Activar logging para mejor depuración
        on_conversation_end=lambda conv_id, reason: logger.info(f"Conversación {conv_id} finalizada: {reason}")
    )

    # Configurar template para el enrutamiento
    template = Template()
    template.set_metadata("type", "chat")
    llm_agent.add_behaviour(llm_behaviour, template)
